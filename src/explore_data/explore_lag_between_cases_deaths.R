...
# Read in CDP data:
dat_inc_cdp <- read_csv('data/formatted/weekly_covid_deaths_by_lk_INCIDENT_CDP.csv')
...
p3 <- ggplot(data = dat_inc_plot_cdp, aes(x = time, y = val, color = metric)) +
...
p4 <- ggplot(data = dat_inc_plot_cdp, aes(x = time, y = val, color = metric)) +
  geom_line() + scale_color_brewer(palette = 'Set1') + theme_classic() +
  geom_vline(xintercept = pks_cases_cdp + 8) +
  geom_vline(xintercept = pks_deaths_cdp + 8, lty = 2)
print(p4)
